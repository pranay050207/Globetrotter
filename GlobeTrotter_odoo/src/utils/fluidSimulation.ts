...
// Based on Pavel Dobryakov's fluid simulation (MIT), tuned for the Flowstate dark aesthetic.
// Produces vivid cyan→blue→violet→magenta ink swirls on a near-black (#04050c) background.
...
export function fluidSimulation(canvas: HTMLCanvasElement): () => void {
    ...
    multipleSplats(34);
    for (let i = 0; i < 8; i++)
        splatStack.push(10 + Math.floor(Math.random() * 10));
    ...
    function render(target: any) {
        ...
        if (config.SHADING) {
            ...
            gl.uniform1i(displayShadingProgram.uniforms.uTexture, density.read.attach(0));
        } else {
            displayProgram.bind();
    ...
    on(window, 'touchstart', (e: TouchEvent) => {
        ...
        for (let i = 0; i < touches.length; i++) {
            ...
            const p = pointers[i];
            const { x, y } = pointerPos(touches[i].clientX, touches[i].clientY);
            p.id = touches[i].identifier; p.down = true; p.x = x; p.y = y;
            p.color = generateColor();
        }
    }, { passive: true });
