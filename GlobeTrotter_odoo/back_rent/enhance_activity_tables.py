...
def enhance_activity_tables():
    """Add new activity-related tables and columns to the database"""
    
    # Connect to the database
    engine = create_engine("sqlite:///globe_trotter.db")
    ...
    with engine.connect() as conn:
        ...
        for column_def in new_columns:
            try:
                ...
                conn.execute(text(f"ALTER TABLE activities ADD COLUMN {column_def}"))
        ...
        try:
            conn.execute(text("UPDATE activities SET category = 'sightseeing' WHERE category IS NULL"))
        ...
        for activity_data in sample_activities:
            try:
                ...
                existing = conn.execute(text(
                    "SELECT id FROM activities WHERE name = :name AND location = :location"
                ), activity_data).fetchone()
